...
import RelationshipsModel from '../relationships/relationships.model.js'
...
class MypageService {
    ...
    static async getUserProfileInfo(serviceId) {
        try {
            ...
            const [interestCount, networkCount] = await Promise.all([
                RelationshipsModel.countSentInterests(BigInt(serviceId)),
                RelationshipsModel.countNetworkConnections(BigInt(serviceId))
            ])
